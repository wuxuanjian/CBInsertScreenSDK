...
-(void) showImage
{
    if([_imageArray count] > [self oneImageItem])
    {
        ...
        [_oneScrollImageView setImageWithURL:url placeholderImage:[UIImage imageNamed:@"hotel_img_bg"]];
    }
    ...
    if([_imageArray count] > [self twoImageItem])
    {
        ...
        NSURL *url = [NSURL URLWithString:urlStr];
        [_twoScrollImageView setImageWithURL:url placeholderImage:[UIImage imageNamed:@"hotel_img_bg"]];
    }
    if([_imageArray count] > [self threeImageItem])
    {
        NSString* urlStr = [_imageArray objectAtIndex:[self threeImageItem]];
        NSURL *url = [NSURL URLWithString:urlStr];
        [_threeScrollImageView setImageWithURL:url placeholderImage:[UIImage imageNamed:@"hotel_img_bg"]];
    }
...
{
    CGFloat dateViewW = screenViewW(screenDirection);
    if (_screenScrollView.contentOffset.x > dateViewW)
    {
        [self focusAdd];
        [self showImage];
    }
    else if (_screenScrollView.contentOffset.x < dateViewW)
    {
        [self focusApp];
        [self showImage];
    }
